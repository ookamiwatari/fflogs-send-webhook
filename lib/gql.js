...
module.exports = {
	...
	init: async () => {
		...
		zones = (await gqlClient.request('{ worldData { zones { id name encounters { id name } } } }')).worldData.zones;
	},
			...
			for (const encounter of encounter_list) {
				for (const rank of character['e'+encounter.encounter_id].ranks) {
					mysql.reserveReport(
						rank.report.code,
						rank.report.fightID,
						character_list.find((char) => char.id === +key.slice(1)).webhook_url,
	...
	getReportData: async (code, fight_id) => {
		...
		const str = '' +
			...
			`【${fight.name}】${fight.gameZone.name} ${('00'+Math.floor(battle_time/60000)).slice(-2)}:${('00'+Math.floor(battle_time/1000)%60).slice(-2)}` +
			`\`\`\`` +
			players.map((p) => {
				return '' +
