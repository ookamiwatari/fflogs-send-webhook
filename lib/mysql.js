...
module.exports = {
  ...
  reserveReport: async (code, fight_id, webhook_url, initialized) => {
    if (
      cached_report_list.find(
        (report) =>
          report.code === code &&
          report.fightId === fight_id &&
          report.webhookUrl === webhook_url
      )
    )
      return;
    cached_report_list.push({
    ...
    const counts = await connection.execute(
      `SELECT COUNT(*) FROM report WHERE code = "${code}" AND fight_id = ${fight_id} AND webhook_url = "${webhook_url}" LIMIT 1`
    );
    const result = await connection.execute(
      `INSERT INTO report (code, fight_id, webhook_url, finished) VALUES ("${code}", ${fight_id}, "${webhook_url}", ${!initialized})`
    );
  },

  getFetchReport: async () => {
    const result = await connection.execute(
      `SELECT * FROM report WHERE finished = 0 AND fetch_date < ${Math.floor(
        new Date().getTime() / 1000 - 120
      )} LIMIT 1`
    );
    if (!result[0].length) return;
    await connection.execute(
      `UPDATE report SET fetch_date = ${Math.floor(
  ...
  successReport: async (id) => {
    return await connection.execute(
      `UPDATE report SET finished = 1 where id=${id}`
    );
  },
};
